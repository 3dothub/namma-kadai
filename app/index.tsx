...
export default function Index() {
  const { currentLocation, hasCompletedWelcome, hasLocationAccess } = useSelector((state: RootState) => state.user);

  // Check if user has location data from either authSlice or productSlice
  const hasLocation = !!(currentLocation || currentLocation || hasLocationAccess);
