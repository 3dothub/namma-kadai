...
export default function OrderScreen() {
  ...
  const activeOrders = orders.filter((order) => ["pending", "confirmed", "dispatched"].includes(order.status));
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.emptyState}>
          ...
          <Text style={styles.emptyStateText}>You need to login to view your orders</Text>
        </View>
      </SafeAreaView>
    );
  }
            ...
            <View key={order._id} style={styles.orderCard}>
              ...
              <View style={styles.orderDetails}>
                ...
                {order.deliveryAddress && (
                  <Text style={styles.orderAddress}>
                    <Ionicons name="location-outline" size={12} color="#6B7280" /> {order.deliveryAddress.street},{" "}
                    {order.deliveryAddress.city}
                  </Text>
                )}
                {order.orderType === "takeaway" && (
                  <Text style={styles.orderAddress}>
                    <Ionicons name="bag-outline" size={12} color="#6B7280" /> Takeaway Order
                  </Text>
                )}
              </View>
              ...
              {!["delivered", "cancelled"].includes(order.status) && (
                <View style={styles.estimatedTime}>
                  ...
                  <Text style={styles.estimatedTimeText}>Estimated delivery: {getEstimatedTime(order.status)}</Text>
                </View>
              )}
            </View>
          ))
